...
export async function POST(request: Request) {
  try {
    ...
    const { role, experiences, skills } = await request.json();

    if (!role && (!experiences || experiences.length === 0)) {
      return NextResponse.json(
        { error: 'Mohon isi setidaknya target profesi atau riwayat pengalaman.' },
        { status: 400 }
      );
    }
    ...
    const summary = await aiProvider.generateSummary(
      role || 'Profesional',
      typeof experiences === 'string' ? experiences : JSON.stringify(experiences || ''),
    ...
    return NextResponse.json(
      { error: 'Gagal menghasilkan ringkasan profil.' },
      { status: 500 }
    );
  }
}
